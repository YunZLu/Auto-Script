...
\033[0;33m--------------------------------------\033[0m"
    ...
    case $option in 
        ...
        2) 
      	...
      	3)
	   		# 添加混音
			speakList=$(cat /root/MM_Audio/config/default.json|jq '.voice_id' | keys | awk -F'"' '{print $2}'| awk 'NF{a++;print "\033[0;33m"a"\033[0m""\033[0;33m.\033[0m","\033[0;33m"$0"\033[0m\n";next}1')
			if [ ! "$speakList" ]; then
				...
				speakName=$(jq --arg n ${speakChose} '.voice_id' | keys | awk -F'"' '{print $2}'| awk NR==$n /root/MM_Audio/config/default.json)
