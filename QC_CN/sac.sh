...
\033[0m\n(⇀‸↼‶)"
    ...
    case $option in 
        ...
        4) 
	    	...
	    	read -n 1 QQchose
                echo -e "\033[0;36m你确定要登录以下QQ号喵？(y|N)\033[0m\n"
		QQnumber=$(QQchose=1;ls -1 /opt/QQ/resources/app/app_launcher/napcat/config/ | awk -F'_' '{print $2}' | awk -F'.' '{print $1}' | awk '!a[$0]++{print}'| awk NF | awk -v QQchose=$QQchose NR==$QQchose)
		read -n 1 chose
		case $chose in 
        	    y|Y)
